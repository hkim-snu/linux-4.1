...
static int link_path_walk(const char *name, struct nameidata *nd)
{
	 ...
	 * hj
	 * name resolution
	 * name: /home/dcslab/abc.txt
	 * after resolve: home/dcslab/abc.txt
	 * remove "/" root
	 */

	//printk("[HJ] @link_path_walk before name resolve [%s]\n", name);
	...
	//printk("[HJ] after name resolve [%s]\n", name);
	...
	for(;;) {
		 ...
		 * hj
		 *
		 * hash_len: 14727773742 | hash with name: mnt/sdb/UrbanZakapa/Urban/Zakapa/albums/2016/test6K.txt
		 * hash_len: 16964386422 | hash with name: sdb/UrbanZakapa/Urban/Zakapa/albums/2016/test6K.txt
		 * hash_len: 50456853123 | hash with name: UrbanZakapa/Urban/Zakapa/albums/2016/test6K.txt
		 * hash_len: 23469128420 | hash with name: Urban/Zakapa/albums/2016/test6K.txt
		 * hash_len: 27144412098 | hash with name: Zakapa/albums/2016/test6K.txt
		 * hash_len: 29083475427 | hash with name: albums/2016/test6K.txt
		 * hash_len: 18906818363 | hash with name: 2016/test6K.txt
		 * hash_len: 43496522926 | hash with name: test6K.txt
		 */

		hash_len = hash_name(name);
		// hj - could be overhead...??
		//printk("[HJ] hash_len: %llu | hash with name: %s\n", hash_len, name);
		...
		if (likely(type == LAST_NORM)) {
			/* 
			 * hj
			 * 여기가 (부모 dentry, 파일이름) 을 키로 하는 hash table 을 찾는 곳이... 아닌가? -.-
			 */
			struct dentry *parent = nd->path.dentry;
			nd->flags &= ~LOOKUP_JUMPED;
			if (unlikely(parent->d_flags & DCACHE_OP_HASH)) {
				...
				/* 
				 * hj
				 * 여기있는 d_hash() 는 무슨 함수가 호출이 되는거지?? 어디서 뭘로 엮여있는거지??
				 */
		     ...
		     struct nameidata *nd)
{
	int retval = 0;
	/*
	 * hj - name->name 은 full path 임. /var/log/syslog 같이.
	 * printk("[path_init] filename: %s\n", s);
	 */
	...
	//printk("[HJ] [%s] path_init() flags: %x\n", name->name, flags);

	nd->last_type = LAST_ROOT; /* if there are only slashes... */
	/*
	 * hj
	 * flags -> file open flag!
	 * 테스트할 때는 "r" 로 했음...
	 */
	...
	if (flags & LOOKUP_ROOT) {
		...
		//printk("[HJ] [%s] if (flags & LOOKUP_ROOT) \n", name->name);
		if (*s) {
			if (!d_can_lookup(root))
				return -ENOTDIR;
			/*
			 * inode_permission -> 0 이면 permission 있는 거고, 아니면 error value return 함.
			 * 0 이 아닌 모든 값은 error, 즉 permission 이 없음. path 따라갈 필요도 없음.
			 * 1) superblock level 에서 permission 확인 -> read-only FS 인지 확인하고 있음.
			 * 2) given inode 에 대해 R/W/X permission 확인하는데...
			 * 여기서는 MAY_EXEC?? x permission 만 확인하는듯? 왜지?? -> LOOKUP_ROOT 라서?
			 */
			retval = inode_permission(inode, MAY_EXEC);		// hj - inode permission 확인.
			if (retval)
				return retval;
		}
		nd->path = nd->root;
	 ...
	 * read only 라면 없어도 되는 부분 아닐까...?? 어차피 수십 ns 밖에 안걸림 -_-;
	...
	if (*s == '/') {
		 ...
		 * hj
		 * 여기로 들어옴. 
		 * do_sys_open(AT_FDCWD, ... ) 으로 호출함. 근데 AT_FDCWD 가 무슨 뜻이지?
		 *
		...
		if (flags & LOOKUP_RCU) {
			...
			/* 
			 * hj
			 * flags = 0x41 or 0x40, LOOKUP_RCU = 0x40
			 * -> means take this routine
			 *
			 * rcu_read_lock() : 25ns
			 */
			rcu_read_lock();
			...
			do {
				...
				/* 
				 * hj
				 * read_seqcount_retry() : end a seq-read critical section (smp_rmb() inside function)
				 * fs->seq, seq 두개가 같아야 loop 빠져나감
				 * retry 해야하면 1 을 return 함.
				 */
			} while (read_seqcount_retry(&fs->seq, seq));
		} else {
			getnstimeofday(&ts);
			...
			getnstimeofday(&ts);
			end_time = timespec_to_ns(&ts);
			//printk("[HJ] [%s] get_fs_pwd() time [%llu]\n", name->name, end_time - start_time);
		}
	} else {
