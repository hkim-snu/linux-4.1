			      ...
			      loff_t offset)
{
	...
	/*
	 * hj 
	 * inode 의 struct ext4_sb_info -> struct journal_s 의 값을 확인하고 있음...
	 * ext4 inode 의 journal mode 확인하는 코드임 -> read-only FS 라면 이 부분 필요 없음.
	...
	/*
	 * hj
	 * inode field check: i_flags
	 * inode 의 0x20 i_flags (__le32) 에서 28번째 bit 값 (EXT4_INLINE_DATA_FL) 이 1인지 여부를
	 * atomic 하게 check 하고 있음. (test_bit 함수로)
	 * -> 즉, 충분히 optimize 되어 있다고 판단됨.
	 */
	if (ext4_has_inline_data(inode))
		return 0;
